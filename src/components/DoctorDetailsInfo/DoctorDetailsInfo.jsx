...
import { formatMedia, formatDate, renderRichTextWithImages } from "@/utils/strapiHelpers";
                ...
                return (
                  ...
                  >
                    <div className="commContent_wrap content-gap-24">
                      <div className="row g-6">
                        <div className="col-lg-7">
                          <div className="content-gap-20">
                            {aboutData?.heading && <span className="contentLabel">{aboutData?.heading || ''}</span>}
                            {aboutData?.subHeading && <h3 className="title-3">{aboutData?.subHeading || ''}</h3>}
                            {!aboutData?.heading && !aboutData?.subHeading && (
                              <>
                                <span className="contentLabel">Deep diving</span>
                                <h3 className="title-3">About the Doctor</h3>
                              </>
                            )}
                            {aboutData?.doctor_image && (
                              <div className="doctors-details-img">
                                <img
                                  src={formatMedia(aboutData?.doctor_image)}
                                  alt={aboutData?.doctor_image?.alternativeText || aboutData?.doctor_name || data?.first_name || ''}
                                />
                              </div>
                            )}
                            {aboutData?.intro_content && (
                              <p className="text-16">{aboutData?.intro_content || ''}</p>
                            )}
                            {aboutData?.detailed_content && (
                              <p className="text-16">{aboutData?.detailed_content || ''}</p>
                            )}
                            {aboutData?.hospitals_description && aboutData?.hospitals_description?.length > 0 && (
                              <div>{renderRichTextWithImages(aboutData?.hospitals_description)}</div>
                            )}
                          </div>
                        </div>
                        <div className="col-lg-5">
                          <div className="content-gap-20">
                            {aboutData?.doctor_name && (
                              <div>
                                <h4 className="f-w-600 mb-2">{aboutData?.doctor_name || ''}</h4>
                              </div>
                            )}
                            {(aboutData?.totalExperience || aboutData?.patientsTreated || aboutData?.registeredNumbered) && (
                              <GridWrapper>
                                {aboutData?.totalExperience && (
                                  <StepCard>
                                    <IconWrapper>
                                    ...
                                    <StepContent>
                                      ...
                                      <StepDescription>Experience</StepDescription>
                                    </StepContent>
                                  </StepCard>
                                )}
                                {aboutData?.patientsTreated && (
                                  <StepCard>
                                    <IconWrapper>
                                ...
                                {aboutData?.registeredNumbered && (
                                  <StepCard>
                                    ...
                                    <StepContent>
                                      ...
                                      <StepDescription>Registration Number</StepDescription>
                                    </StepContent>
                                  </StepCard>
                                )}
                                {aboutData?.visitingDays && Array.isArray(aboutData?.visitingDays) && aboutData?.visitingDays?.length > 0 && (
                                  <StepCard>
                                    <IconWrapper>
                                    ...
                                    <StepContent>
                                      ...
                                      <ul className="day-list">
                                        {aboutData?.visitingDays?.map((day, idx) => (
                                          <li key={idx}>{day || ''}</li>
                                        ))}
                                      </ul>
                                    </StepContent>
                      ...
                      {aboutData?.hospitals && Array.isArray(aboutData?.hospitals) && aboutData?.hospitals?.length > 0 && (
                        <div className="row g-4">
                          <div className="col-lg-7">
                            <div className='doctorsDetails_slider_wrapper'>
                              ...
                              >
                                {aboutData?.hospitals?.map((hospital, index) => (
                                  <SwiperSlide key={hospital?.id || hospital?.documentId || index}>
                                    <div className='doctorsDetails_info_slider'>
                                      {hospital?.featuredImage && (
                                        <img 
                                          src={formatMedia(hospital?.featuredImage)} 
                                          alt={hospital?.featuredImage?.alternativeText || hospital?.name || ''} 
                                        />
                                      )}
                                      <div className='doctorsDetails_info_slider_content'>
                                        <div className='inner_container'>
                                          <div className='commContent_wrap'>
                                            {hospital?.name && <h5>{hospital?.name || ''}</h5>}
                                            {hospital?.description && <p className="text-14">{hospital?.description || ''}</p>}
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                          ...
                          <div className="col-lg-5">
                            {aboutData?.hospitals?.[0] && (
                              <SingleCard>
                                ...
                                <SingleCardTitle className='title-5 text_theme_dark mb-3'>{aboutData?.hospitals?.[0]?.name || ''}</SingleCardTitle>
                                <SingleCardDescription className='text-16 text_theme_dark'>
                                  {aboutData?.hospitals?.[0]?.description || ''}
              ...
              return (
                ...
                >
                  <div className={`commContent_wrap content-gap-24 ${sectionId === 'procedures_performed' ? 'text-white' : ''}`}>
                    ...
                    {sectionId === 'specialization' && sectionData?.specialities && Array.isArray(sectionData?.specialities) && sectionData?.specialities?.length > 0 && (
                      <div className="content-gap-20 mt-4">
                        {sectionData?.specialities?.map((speciality, idx) => (
                          <div key={speciality?.id || idx} className="mb-4">
                            {speciality?.heading && <h5 className="mb-3">{speciality?.heading || ''}</h5>}
                            {speciality?.description && <p className="text-16 mb-3">{speciality?.description || ''}</p>}
                            {speciality?.specialties && Array.isArray(speciality?.specialties) && speciality?.specialties?.length > 0 && (
                              <div className="content-gap-16">
                                {speciality?.specialties?.map((spec, specIdx) => (
                                  <div key={spec?.id || specIdx} className="mb-3">
                                    {spec?.title && <h6 className="mb-2">{spec?.title || ''}</h6>}
                                    {spec?.points && Array.isArray(spec?.points) && spec?.points?.length > 0 && (
                                      <ul className="comm_ul">
                                        {spec?.points?.map((point, pointIdx) => (
