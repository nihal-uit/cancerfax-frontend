...
const WhyOpt = ({ sectionClass, data, loading }) => {
  ...
  const content = data
    ? {
        ...
        cards: data?.cards.length > 0 ? data?.cards : whyOptContent,
      }
    : {
        label: "Lorem Ipsum 2",
          ...
          <HeaderWrapper>
            <Header className="commContent_wrap">
              <Label className="contentLabel">
                {content.label || "Why CAR T-Cell Therapy"}
              </Label>
            ...
            <ExploreButton className="btn btn-pink-solid" as={content.buttonLink ? "a" : "button"} href={content.buttonLink} target={content.buttonTarget}>
              {content.buttonText}
            </ExploreButton>
          </HeaderWrapper>
