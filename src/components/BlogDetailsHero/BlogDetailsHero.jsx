...
const BlogDetailsHero = ({ data, loading }) => {
  ...
  const content = useMemo(() => {
    ...
    if (externalVideo) {
      ...
      // Use featuredImage as poster/thumbnail if available
  ...
  const renderMedia = () => {
    if (content.mediaType === 'video' && content.videoUrl) {
      return (
        <>
          {content.isYouTube ? (
            // For YouTube videos, show poster image as background
            ...
            // For uploaded videos, show video playing in background (muted, looped)
            <>
              <BackgroundVideo
                className="video"
                preload="auto"
                ...
                muted
                playsInline
                poster={content.backgroundImage || undefined}
              >
                <source src={content.videoUrl} type="video/mp4" />
              </BackgroundVideo>
              {/* Optional: Show poster overlay that fades out on hover */}
              {content.backgroundImage && (
                <PosterOverlay
                  src={content.backgroundImage}
                  alt={content.blogTitle}
